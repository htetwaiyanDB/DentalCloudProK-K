...
-- Safe to run multiple times. Existing behavior is preserved by the one-time
-- backfill before the column becomes NOT NULL.
BEGIN;
...
UPDATE public.doctors SET commission_per_visit = 0 WHERE commission_per_visit IS NULL;

ALTER TABLE public.doctors DROP CONSTRAINT IF EXISTS doctors_commission_per_visit_check;
ALTER TABLE public.doctors ADD CONSTRAINT doctors_commission_per_visit_check
  CHECK (commission_per_visit >= 0) NOT VALID;
ALTER TABLE public.doctors VALIDATE CONSTRAINT doctors_commission_per_visit_check;
ALTER TABLE public.doctors
...
SELECT
  ...
  COUNT(*) FILTER (WHERE commission_type = 'flat_visit') AS fixed_doctors,
