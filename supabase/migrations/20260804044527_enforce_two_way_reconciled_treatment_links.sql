...
BEGIN
  ...
  IF jsonb_typeof(NEW.receipt_snapshot -> 'treatments') IS DISTINCT FROM 'array'
     OR jsonb_typeof(NEW.receipt_snapshot -> 'medicines') IS DISTINCT FROM 'array' THEN
  ...
  IF ABS(NEW.amount - (v_treatment_total + v_medicine_total + v_service_fee)) > 0.005 THEN
    RAISE EXCEPTION 'Payment amount does not equal reconciled receipt allocation';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE NOT EXISTS (
      SELECT 1
      FROM unnest(COALESCE(NEW.treatment_ids, ARRAY[]::UUID[])) AS linked_id
      WHERE linked_id::TEXT = item ->> 'id'
    )
  ) THEN
    RAISE EXCEPTION 'Receipt snapshot treatment is missing from linked treatment IDs';
  END IF;
